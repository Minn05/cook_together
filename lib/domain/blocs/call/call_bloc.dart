...
class CallBloc extends Bloc<CallEvent, CallState> {
  ...
  void initSocketChat() async {
    ...
    _socket.on('accept-calling', (data) {
      print("=======================================ON data" +
          data['sessionOffer']);
      print(
          "=======================================ON data" + data['callerId']);
      print(
          "=======================================ON data" + data['receiveId']);
      print("=======================================ON data" +
          data['isOffer'].toString());
      add(OnCallVideoRTCStateEvent(
          data['sessionOffer'],
          data['sessionAnswer'],
          data['candidate'],
          data['callerId'],
    ...
    _socket.emit('call-video-rtc', {'test': 'test'});
  }
  ...
  Future<void> _emitCalling(
      OnEmitCallingEvent event, Emitter<CallState> emit) async {
    _socket.emit('call-video-rtc', {});
  }

  Future<void> _callingVideoRTC(
      OnCallVideoRTCEvent event, Emitter<CallState> emit) async {
    _socket.emit('call-video-rtc', {
  ...
  Future<void> _callingVideoStateRTC(
      OnCallVideoRTCStateEvent event, Emitter<CallState> emit) async {
  ...
  Future<void> _cancelVideoCall(
      OnCancelVideoCallEvent event, Emitter<CallState> emit) async {
    emit(CancelCallVideoState(isCancel: event.cancel));
  }

  Future<void> _onResetCall(
      OnResetCallEvent event, Emitter<CallState> emit) async {

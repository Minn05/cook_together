...
class CallScreenRTC extends StatefulWidget {
  const CallScreenRTC(
      {super.key, required this.callerId, required this.receiverId});
...
class _CallScreenRTCState extends State<CallScreenRTC> {
                    ...
                    Container(
                      child: BlocBuilder<CallBloc, CallState>(
                        builder: (_, state) {
                          ...
                          print('======================================');
                          if (state.receiverId == user.user!.uid)
                            return Text("Offer");
                          return Button(
                              onPress: () async {
                                dynamic session =
                                    await jsonDecode('${state.sessionOffer}');
                                String sdp = write(session, null);
                                RTCSessionDescription description =
                                    RTCSessionDescription(sdp,
                                        state.isOffer ? 'answer' : 'offer');
                                ...
                                var sessionAnswer =
                                    parse(descriptionAnswaer.sdp as String);
                                print(jsonEncode(sessionAnswer));
                                callBloc.add(OnCallVideoRTCEvent(
                                    state.sessionOffer as String,
                                    jsonEncode(sessionAnswer),
                                    '',
                                    state.callerId as String,
                                    state.receiverId as String,
                                    true));
                                _peerConnection!
                                    .setLocalDescription(descriptionAnswaer);
                                setState(() {});
                              },
                              height: 60,
                              width: 60,
  ...
  Future<void> _getOffer() async {
    ...
    callBloc.add(OnCallVideoRTCEvent(
        jsonEncode(session), '', '', widget.callerId, widget.receiverId, true));
